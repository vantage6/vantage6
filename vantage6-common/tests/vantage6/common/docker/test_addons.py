...
# These tests are adapted from [0]. We do this because parse_image_name use
# internal docker-py functions, which may change without notice in future
# versions.
# [0]: https://github.com/docker/docker-py/blob/9ad4bddc9ee23f3646f256280a21ef86274e39bc/tests/unit/auth_test.py#L27s
class TestParseImageName(TestCase):
    ...
    def test_parse_image_name_dotted_hub_library_image(self):
        self.assertEqual(
            parse_image_name('image.valid'),
            ('docker.io', 'image.valid', 'latest')
        )
    ...
    def test_parse_image_name_explicit_hub_library_image(self):
        self.assertEqual(
            parse_image_name('docker.io/image'),
            ('docker.io', 'image', 'latest')
        )

    def test_parse_image_name_explicit_legacy_hub_library_image(self):
        self.assertEqual(
            parse_image_name('index.docker.io/image'),
            ('docker.io', 'image', 'latest')
        )
    ...
    def test_parse_image_name_localhost(self):
        self.assertEqual(
            parse_image_name('localhost/image'),
