...
def compute_job_pod_run_status(
        ...
        if pod_phase == "Pending":
            ...
            elif pending_status_reason in pod_initialization_related_reasons_for_still_pending:
                log.debug(
                    "Job POD (label %s, namespace %s) - Reporting INITIALIZING status: %s",
                    label,
                    task_namespace,
                    pod_initialization_related_reasons_for_still_pending[pending_status_reason],
                )
