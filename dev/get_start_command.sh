...
# This script checks if the vantage6 hub has already been populated with example data.
# If not, it prompts the user to populate the hub and runs the necessary devspace pipeline.
# It uses a marker file to track whether the hub has been populated.
...
if [ "$populate" = "true" ]; then
  echo "Running devspace pipeline with populating the hub..." >&2
  ...
  echo "Running devspace pipeline without populating the hub..." >&2
