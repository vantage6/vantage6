...
def _linux_path_as_str(path: Path) -> str:
    """
    Convert a path, either Linux or Windows, to a Linux-style string path.
...
def replace_wsl_path(path: Path, to_mnt_wsl: bool = True) -> Path:
    ...
    if to_mnt_wsl and _linux_path_as_str(path).startswith(wsl_reference_path):
        path = Path(wsl_regular_path) / path.relative_to(wsl_reference_path)
    elif not to_mnt_wsl and _linux_path_as_str(path).startswith(wsl_regular_path):
...
def str_replace_wsl_path(path: Path, to_mnt_wsl: bool = True) -> Path:
    print("replace again", _linux_path_as_str(replace_wsl_path(path, to_mnt_wsl)))
    return _linux_path_as_str(replace_wsl_path(path, to_mnt_wsl))