.. _ui:

User interface
--------------

The User Interface (UI) is a website where you can login with your vantage6
user account. Which website this is, depends on the vantage6 hub you are
using. If you are using the Uluru community service, go to
https://portal.uluru.vantage6.ai and login with your user account.

Using the UI should be relatively straightforward. If anything is unclear, please
contact us via `Discord <https://discord.com/invite/yAyFf6Y>`__.

.. figure:: /images/ui-screenshot.png

    Screenshot of the vantage6 UI

.. note::
    If you are a hub administrator and want to set up a user interface, see
    :ref:`this section <install-ui>` on deploying a UI.

.. note::
    If you are :ref:`running your own local vantage6 network <use-hub>` with ``v6 sandbox``
    or ``v6 dev``, the UI is started automatically. How to use
    these commands is explained :ref:`here <local-test>`.
