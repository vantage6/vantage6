...
Vantage6 algorithms are by default isolated in their own private network and thus
disconnected from the internet, and are therefore unable to connect to access data that
is not connected to the node on node startup. Via the whitelisting feature, it is
possible to whitelist certain Kubernetes
